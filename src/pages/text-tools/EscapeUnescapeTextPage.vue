...
const switchText = ref(switchValue.value === '0' ? 'Unescape' : 'Escape')
const input = ref('')
const output = ref('')
...
watch(switchValue, () => {
  ...
  if (switchValue.value === '0') {
    ...
    output.value = unescapeText(input.value)
  }
  else{
...
watch(input, (newValue) => {
  if (switchValue.value === '0') {
    output.value = unescapeText(newValue)
  }
  else{
    output.value = escapeText(newValue)
  }
